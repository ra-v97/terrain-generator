...
import transformation.TransformationP8;

import static org.junit.jupiter.api.Assertions.*;

public class TransformationP8Test {
    private Transformation transformation = new TransformationP8();
    ...
    @Test
    public void conditionFailsWithAllBorderEdges() {
        ModelGraph graph = createAllBorderEdgesGraph();
    ...
    @Test
    public void conditionFailsWithIncorrectHangingNodes() {
        ModelGraph graph = createIncorrectHangingNodesGraph();
        assertFalse(transformation.isConditionCompleted(graph, graph.getInterior("i1").orElseThrow(AssertionError::new)));
    }
    ...
    public void transformationProducesCorrectVertexTypes() {
        ...
        int simpleNodes = (int) graph.getVertices()
                ...
                .count();
        assertEquals(3, hangingNodes, simpleNodes);
    }
    ...
    public void transformationProducesCorrectEdges() {
        ...
        assertFalse(edge1.getB() || edge2.getB() || edge3.getB());
    }
    ...
    @Test
    public void transformationProducesOneNewHangingNode() {
        ModelGraph graph = createCorrectGraph();
        InteriorNode interior = graph.getInterior("i1").orElseThrow(AssertionError::new);
        transformation.transformGraph(graph, interior);
    ...
    private ModelGraph createAllBorderEdgesGraph() {
        ModelGraph graph = new ModelGraph("testGraph");
        Vertex v1 = graph.insertVertex("v1", VertexType.SIMPLE_NODE, new Point3d(0.0, 0.0, 0.0));
        Vertex h2 = graph.insertVertex("h2", VertexType.HANGING_NODE, new Point3d(4.0, 0.0, 0.0));
        Vertex v3 = graph.insertVertex("v3", VertexType.SIMPLE_NODE, new Point3d(8.0, 0.0, 0.0));
    ...
    private ModelGraph createIncorrectHangingNodesGraph() {
        ModelGraph graph = new ModelGraph("testGraph");
        Vertex v1 = graph.insertVertex("v1", VertexType.SIMPLE_NODE, new Point3d(0.0, 0.0, 0.0));
        Vertex h2 = graph.insertVertex("h2", VertexType.HANGING_NODE, new Point3d(4.0, 0.0, 0.0));
        Vertex v3 = graph.insertVertex("v3", VertexType.SIMPLE_NODE, new Point3d(8.0, 0.0, 0.0));
        Vertex v5 = graph.insertVertex("v5", VertexType.SIMPLE_NODE, new Point3d(6.0, 8.0, 0.0));
        Vertex h6 = graph.insertVertex("h6", VertexType.HANGING_NODE, new Point3d(3.0, 4.0, 0.0));

        graph.insertEdge("e1", v1, h2, true);
        graph.insertEdge("e2", h2, v3, true);
        graph.insertEdge("e3", v3, v5, true);
        graph.insertEdge("e4", v5, h6, false);
        graph.insertEdge("e5", h6, v1, false);

        graph.insertInterior("i1", v1, v3, v5, h2, h6);
        return graph;
    }
}
